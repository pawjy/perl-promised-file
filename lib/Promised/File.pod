...
=item $file->utime ($atime, $mtime)->then (...)

Change the file's timestamps to the specified value.  The arguments
are same as C<utime>'s second and third arguments, i.e. access and
modified times in Unix time seconds.
